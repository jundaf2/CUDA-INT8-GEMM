# 1 "/home/poweruser/junda.feng/CUDA-INT8-GEMM/test_gemm_i8.cu"
extern struct __C8 *__curr_eh_stack_entry;
extern unsigned short __eh_curr_region;
static const char __T95[48];
static const char __T96[40];
# 61 "/usr/include/c++/7/iostream" 3
extern _ZSt7ostream _ZSt4cout __attribute__((visibility("default")));
# 74 "/usr/include/c++/7/iostream" 3
static struct _ZNSt8ios_base4InitE _ZN46_INTERNAL_7a73785f_15_test_gemm_i8_cu_46d8dfc7St8__ioinitE __attribute__((visibility("default"))) = {};
extern void *__dso_handle __attribute__((visibility("hidden")));
static const char __T95[48] = "void GEMM<T>::testGEMM() [with T = signed char]";
static const char __T96[40] = "void GEMM<T>::testGEMM() [with T = int]";
